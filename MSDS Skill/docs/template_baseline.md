...
- EN active baseline: `examples/template_reference_en.docx` (fresh clone of the source with the approved Section 8.2 child table and one targeted label-residual cleanup)
...
CN SHA-256: `2e03e8826219f94724281ec415874875420991f34e03d9eab0d8731a16bee969`
...
- Section 8.2's writable value cell now contains one maintained nested child table with 2 rows × 4 columns. The locked header is `物质 / 依据 / 类型 / 数值` in CN and `Substance / Basis / Type / Value` in EN. The child grid is `[2400, 1100, 1100, 1600]` twips (`[1524000, 698500, 698500, 1016000]` EMU as exposed by python-docx), which fits the 8.2 value cell and keeps the fourth column visible in WPS/Word; the parent row is non-splittable at page boundaries. The initial second row is a blank styled data slot; additional verified source records are cloned in place.
...
- `tests/template_snapshot.json` and `tests/template_snapshot_en.json` record the active CN/EN table/cell merges, grid and cell widths, recursive child-table geometry, paragraph/run properties, and header/footer parts. Versioned v3.13 copies are retained alongside historical v3.12 evidence.
