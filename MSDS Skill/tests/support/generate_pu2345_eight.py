...
SOURCE = PACKAGE / "examples" / "regression_EP-1704_source.docx"
TEMPLATE = PACKAGE / "examples" / "template_reference.docx"
OUT_ROOT = PACKAGE / "artifacts" / "package-msds-overwrite-agent" / "runs" / "test-pu2345"
...
def set_row(row, values):
    ...
    # S11/S12 structured rows are physically two-column rows even though
    # their semantic payload may carry label, sublabel and value. Preserve
    # the semantic detail by folding the non-label cells into the visible
    # merged value cell when the row has only two physical cells.
    if len(cells) == 2 and len(values) > 2:
        values = [values[0], "\n".join(str(value) for value in values[1:] if str(value).strip())]
...
def write_body(doc: Document, facts: dict, language: str):
    ...
    headings = HEADINGS_EN if language == "en" else HEADINGS_CN
    for ti, heading in enumerate(headings, 1):
        set_cell_text(unique_cells(doc.tables[ti - 1].rows[0])[0], heading)
...
def write_header_footer(doc: Document, language: str, brand: str):
    ...
    for section in doc.sections:
        ...
        for table in section.footer.tables:
            ...
            if first:
                set_cell_text(first[0], f"{company}  PU-2345-MSDS")
                if len(first) > 1:
...
def build_one(language: str, brand: str):
    ...
    if language == "zh":
        ...
        # Footer normalization is language-neutral; only the table-body
        # compaction remains CN-only.
    ...
    audit = {
        ...
        "template_geometry": {"table_count": 16, "rows": [10, 16, 6, 6, 5, 4, 3, 12, 24, 6, 18, 6, 3, 5, 9, 2]},
