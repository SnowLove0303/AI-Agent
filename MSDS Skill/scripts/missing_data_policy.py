...
def apply_source_absence_policy(document, facts: dict, unique_cells) -> dict:
    ...
    for section in (4, 5, 6, 7, 10, 13, 14):
        ...
        for index, row in reversed(list(enumerate(table.rows[1:], 1))):
            ...
            if not absent and len(cells) > 1:
                absent = all(not cell.text.strip() for cell in cells[1:])
            if absent:
    ...
    for section in (11, 12):
        ...
        has_endpoint = any(
            row_state(row) == SourceState.SUPPORTED and len(row) > 1
            for row in rows[1:]
        )
        table = document.tables[section - 1]
        if not has_endpoint:
