...
LOCAL_SECTION_POLICIES = {
    ...
    11: {"match_basis": "endpoint_study_field", "source_text_policy": "verbatim_structured_source_fields; in 11.1/11.2/11.7 three-column rows write only the final value cell; strip only a repeated endpoint/child heading already owned by the locked label cells; preserve the remaining source wording; renumber surviving endpoint groups continuously after omission", "layout_policy": "first two columns are locked bold labels; every value is explicit language font/12pt, vertical center, left align", "empty_policy": "align_skeleton_then_hide_absent_endpoint"},
