...
TDS 当前正式基线为四个用户提供的中英文/冠志国彩模板，版本 `1.3.6`。模板来源、active DOCX、注册表和结构快照均在本目录内留有证据。性能表数据行按源文件顺序保真写入，模板数据行只提供锁定样式；不足时裁剪未使用示例行，超出时按白名单克隆扩展。产品特性段落保留模板自动编号，输出统一采用紧凑悬挂式布局，编号和正文基准线由代码固定校正，空编号段不参与列表布局；槽位和分隔段由注册表按 active 模板动态识别，扩展项插入原有分隔段之前。
