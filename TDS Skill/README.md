...
TDS 当前正式基线为四个用户提供的中英文/冠志国彩模板，版本 `1.3.1`。模板来源、active DOCX、注册表和结构快照均在本目录内留有证据。性能表数据行按源文件顺序保真写入，模板数据行只提供锁定样式；不足时裁剪未使用示例行，超出时按白名单克隆扩展。两个基线特性段落也可按白名单克隆扩展，格式骨架仍以对应模板为唯一来源。
