...
TDS 当前正式基线为四个用户提供的中英文/冠志国彩模板，版本 `1.3.5`。模板来源、active DOCX、注册表和结构快照均在本目录内留有证据。性能表数据行按源文件顺序保真写入，模板数据行只提供锁定样式；不足时裁剪未使用示例行，超出时按白名单克隆扩展。产品特性段落保留模板自动编号，槽位和分隔段由注册表按 active 模板动态识别；基线与扩展项继承模板的行距、段前后距和字符格式，不能由覆写程序重建或猜测格式。
