...
- 模板严格性门禁：每个字段只能写入 active 模板已注册的值槽；任何标题居中、缩进/间距重置、编号/制表位重写、空白段落清理或非模板形状的重排都属于越权格式修改，禁止自动执行。槽位容量不足时，只能按 registry 的扩展规则深拷贝对应模板段落/表格行的完整格式，再写入新增值；不得为了“排好看”修改模板框架。
...
- 机器审计必须把 active 模板与输出的段落/字符/表格形状逐项比对；允许的例外只有值槽文本替换、registry 声明的同形状段落/数据行克隆、经 decision ledger 批准的整段隐藏，以及 registry 声明的文末空白占位收束。新增段落/行的形状不是对应模板槽位的完整克隆，或标题样式、编号、缩进、间距和制表位被重设时必须 `RELEASE_FAIL`。
