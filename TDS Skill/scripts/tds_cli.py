...
def main():
    ...
    if x.cmd=='build':
        ...
        audit=x.output_dir/'audit'; audit.mkdir(parents=True,exist_ok=True)
        ...
        run('overwrite_tds.py','--mapping',mapping,'--registry',ROOT/'mapping'/'template_field_registry.json','--output-dir',x.output_dir,'--model',x.model); run('convert_docx_to_pdf.py','--input-dir',x.output_dir); run('audit_tds_eight.py','--output-dir',x.output_dir,'--registry',ROOT/'mapping'/'template_field_registry.json','--mapping',mapping,'--model',x.model,'--report',x.output_dir/'audit'/'release_report.json'); return
